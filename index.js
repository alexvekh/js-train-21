...
class Guitarist extends Musician {
  ...
  set band(band) {
    this.#band = band; // присвоює нове значення приватному полю #band
  }
...
class Band {
  ...
  addMember(newMember) {
    ...
    } else console.log("Новий учасник повинен бути екземпляром класу Musician");
  }
  ...
  playMusic() {
    this.#members.forEach((musician) => musician.play());
  }
}
//addMember(newMember);
...
class SongWriter {
  ...
  info() {
    if (this.songs.length % 10 === 1 && this.songs.length !== 11) console.log(`Написав ${this.songs.length} пісню`);
    else if ((this.songs.length % 10 === 2 && this.songs.length !== 12) || (this.songs.length % 10 === 3 && this.songs.length !== 13))
      console.log(`Написав ${this.songs.length} пісні`);
    else console.log(`Написав ${this.songs.length} пісень`);
  }
}
 ...
 * | songs       | ["Yesterday","Hey Jude","Let It Be",]|
 */
const leadsinger = new LeadSinger("Mick Jager", "The Rolling Stones", ["Yesterday", "Hey Jude", "Let It Be"]);
